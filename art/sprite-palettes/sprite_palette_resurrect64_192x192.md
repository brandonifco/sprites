# RESURRECT 64 SPRITE PROFILE — 192×192
...
- Canvas MUST be exactly `192×192`.
- Unique opaque color count MUST be `24` to `40` inclusive.
- Major material ramp depth MUST be `4` to `6` colors inclusive.
- Minor material ramp depth MUST be `1` to `3` colors inclusive.
- Exterior contour thickness MUST be `1` or `2` pixels. A `2`-pixel contour segment MUST be limited to local dark masses and MUST NOT form a uniform border.
- Dithering is allowed only on rough materials. Dithered area MUST NOT exceed `15%` of opaque pixels.
- `R64-09` usage MUST NOT exceed `48` opaque pixels.
- The occupied subject bounding box height MUST be `154` to `184` pixels inclusive unless the asset specification explicitly defines a non-upright subject.
- The largest single highlight cluster MUST NOT exceed `16` connected pixels.
- The sprite MUST contain at least `5` distinct value levels across the full figure.
- The sprite MUST contain at least `2` clear separation edges between body and primary/secondary equipment if such equipment is present.
- Fine texture clusters MUST NOT exceed `4×4` unless the cluster describes a major form edge or rough material patch.
